    ...
    std::pair<CarKeyListener::CarActionMove,CarKeyListener::CarActionTurn> actions) {
     switch (actions.first) {
         ...
         case CarKeyListener::CarActionMove::NOTHING:
                 speed_ *= 1-0.05*deltaTime; // Friction factor
        ...
        model_->position +=(Vector3{speed_*std::sin(angle_),0,speed_*std::cos(angle_)}* static_cast<float>(deltaTime));


        // Apply friction to gradually reduce speed when not accelerating
std::cout << model_->position.x<<" "<< model_->position.y<<" "<< model_->position.z << std::endl;
        }
